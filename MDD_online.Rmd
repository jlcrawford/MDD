...
NCS.path <- "~/Box Sync/CCPLab_Aging_Studies/Multi-Domain-Discounting/Data/Online/Questionnaires/MDD_NCS.csv"
SSQ.path <- "~/Box Sync/CCPLab_Aging_Studies/Multi-Domain-Discounting/Data/Online/Questionnaires/MDD_ssq.csv"
SPSRQ.path <- "~/Box Sync/CCPLab_Aging_Studies/Multi-Domain-Discounting/Data/Online/Questionnaires/MDD_Spsrq.csv"
BISBAS.path <- "~/Box Sync/CCPLab_Aging_Studies/Multi-Domain-Discounting/Data/Online/Questionnaires/MDD_Bisbas.csv"
GRAPES.path <- "~/Box Sync/CCPLab_Aging_Studies/Multi-Domain-Discounting/Data/Online/Questionnaires/MDD_Grapes.csv"
#Working Memory Capacity
LSpan.path <- "~/Box Sync/CCPLab_Aging_Studies/Multi-Domain-Discounting/Data/Online/Listening-Span/MDD_Listening-Span.csv"
OSpan.path <- "~/Box Sync/CCPLab_Aging_Studies/Multi-Domain-Discounting/Data/Online/O-Span/MDD_O-Span.csv"
SymmSpan.path <- "~/Box Sync/CCPLab_Aging_Studies/Multi-Domain-Discounting/Data/Online/Symm-Span/MDD_Symm-Span.csv"
...
## Generate list of usable data from overview sheet

``` {r Participants, warning = F, message = F}
MDD.overview.path <- "~/Box Sync/CCPLab_Aging_Studies/Multi-Domain-Discounting/Data/Online/ID-Lists/MDD-online-participant-log.csv"
MDD.participants <- read.csv(MDD.overview.path, header = T) %>%
  filter(Complete == "Yes") %>% select(Prolific_ID)
MDD.list <- sapply(MDD.participants, as.character)
```
...
NCS.clean <- NCS %>% select(c(subject, ends_with("response"))) %>% filter(subject %in% MDD.list) %>%
...
SSQ.clean <- SSQ %>% select(c(subject, ends_with("response"))) %>% filter(subject %in% MDD.list) %>%
    filter(duplicated(subject)) %>%
...
BISBAS.clean <- BISBAS %>% select(c(subjectid, BAS_Drive, BAS_Fun, BAS_Reward, BIS)) %>%
    filter(duplicated(subjectid))

SPSRQ.clean <- SPSRQ %>% select(c(subjectid, BAS_Drive, BAS_Fun, BAS_Reward, BIS)) %>%
    filter(duplicated(subjectid))

GRAPES.clean <- GRAPES %>% select(c(subject, ends_with("response"))) %>% 
    filter(duplicated(subject)) %>%
