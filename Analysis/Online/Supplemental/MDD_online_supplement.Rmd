...
demo.path <- "~/Box/CCPLab_Aging_Studies/Multi-Domain-Discounting/Data/Online/Questionnaires/MDD_demo.csv"
...
#Make data frame for usable subject info
Subjects <- read.csv(subjects.path, header = F)
```

### Demographic info for sample

```{r Demo, warning=F, message=F}
colnames(Subjects) <- "subjectid"
demo.clean <- Demo %>% rename(subjectid = "subject") %>% select(subjectid, contains("_response")) %>% inner_join(Subjects)
demo.race <- demo.clean %>% group_by(race_response) %>% count(race_response)
demo.ethnicity <- demo.clean %>% group_by(ethnicity_response) %>% count(ethnicity_response)
demo.sex <- demo.clean %>% group_by(sex_response) %>% count(sex_response)
demo.age.mean <- mean(demo.clean$age_response)
demo.age.sd <- sd(demo.clean$age_response)
demo.age.range <- range(demo.clean$age_response)
```
...
m.SV.coged <- brm(data = d.coged.SV, SV ~ taskCode*domainCode + (1 | subjectid), file = "models/m.SV.coged.rds")
summary(m.SV.coged)
...
#Multilevel model of subjective value with task and domain as predictors
m.SV.coged.full <- brm(data = d.coged.SV.full, SV ~ taskCode*domainCode*rewardCode + (1 | subjectid), file = "models/m.SV.coged.full.rds")
...
#T-test between average SV estimates across WM and speech domains
...
m.SV.wm.partial <- brm(data = d.coged.wm.partial, meanSV ~ taskCode + hitrate + CRrate + meanRT + (1 | subjectid), file = "models/m.SV.wm.partial.rds")
...
m.SV.speech.partial <- brm(data = d.coged.speech.partial, meanSV ~ taskCode + performance + (1 | subjectid), file = "models/m.SV.speech.partial.rds")
...
m.SV.speech.partial.test <- brm(data = d.coged.speech.partial.test, meanSV ~ taskCode + performance + WMC.composite + Rew.composite + (1 | subjectid), file = "models/m.SV.speech.partial.test.rds")
...
m.SV.wm.partial.outlier <- brm(data = d.coged.wm.partial.outlier, meanSV ~ taskCode + hitrate + CRrate + meanRT + (1 | subjectid), file = "models/m.SV.wm.partial.outlier.rds")
...
m.SV.speech.partial.outlier <- brm(data = d.coged.speech.partial.outlier, meanSV ~ taskCode + performance + (1 | subjectid), file = "models/m.SV.speech.partial.outlier.rds")
...
m.SV.wm.partial.outlier.plot <- brm(data = d.coged.wm.partial.outlier.plot, meanSV ~ taskCode + hitrate + CRrate + meanRT + WMC.composite + Rew.composite + (1 | subjectid), file = "models/m.SV.wm.partial.outlier.plot.rds")
...
m.SV.speech.partial.outlier.plot <- brm(data = d.coged.speech.partial.outlier.plot, meanSV ~ taskCode + performance + WMC.composite + Rew.composite + (1 | subjectid), file = "models/m.SV.speech.partial.outlier.plot.rds")
...
m.mentalDemand <- brm(data = NASA.m.demand, rating ~ taskCode*domainCode + (1 | subjectid), file = "models/m.mentalDemand.rds")
...
m.Frustration <- brm(data = NASA.frust, rating ~ taskCode*domainCode + (1 | subjectid), file = "models/m.Frustration.rds")
...
m.Effort <- brm(data = NASA.effort, rating ~ taskCode*domainCode + (1 | subjectid), file = "models/m.Effort.rds")
summary(m.Effort)
...
### Comparing performance on N-back and WMC measures

```{r WMC_Nback, message=FALSE, warning=FALSE}
Nback.perf.subj <- performance.wm %>% select(subjectid, task, hitrate, CRrate) %>%
  inner_join(performance.wm.RT.sum, by = c("subjectid", "task")) %>%
  group_by(subjectid) %>% summarise(meanRT = mean(meanRT), meanHR = mean(hitrate), meanCR = mean(CRrate)) %>%
  inner_join(WMC, by = "subjectid")

#Testing the correlation between N-back performance indicies and WMC composite
HitRate.WMC.cor <- cor_test(data = Nback.perf.subj, x = "meanHR", y = "WMC.composite", bayesian = TRUE, bayesian_prior = 0.707107)
HitRate.WMC.cor
CRrate.WMC.cor <- cor_test(data = Nback.perf.subj, x = "meanCR", y = "WMC.composite", bayesian = TRUE, bayesian_prior = 0.707107)
CRrate.WMC.cor
meanRT.WMC.cor <- cor_test(data = Nback.perf.subj, x = "meanRT", y = "WMC.composite", bayesian = TRUE, bayesian_prior = 0.707107)
meanRT.WMC.cor
```
